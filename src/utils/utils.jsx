...
export const formatTimeDiff = (salesEnd) => {
    if (!salesEnd) return '마감일 없음';
    ...
    return `공구마감 ${hours}:${minutes}:${seconds}`;
};
export function GongguTimer({ salesEnd, timerType }) {
    const [timeLeft, setTimeLeft] = useState(() => timeAgoOrClosed(salesEnd));
    
    const getInitialTime = () => {
        if (timerType === 1) return formatTimeDiff(salesEnd);
        if (timerType === 2) return timeAgoOrClosed(salesEnd);
        return '';
    };
    ...
    useEffect(() => {
        const timer = setInterval(() => {
            if (timerType === 1) {
                setTimeLeft(formatTimeDiff(salesEnd));
            } else if (timerType === 2) {
                setTimeLeft(timeAgoOrClosed(salesEnd));
            }
        }, 1000);
